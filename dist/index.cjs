...
var import_proxy_addr2 = __toESM(require("proxy-addr"), 1);
var import_netparser3 = require("netparser");

// src/easy-waf/utils.ts
var import_proxy_addr = __toESM(require("proxy-addr"), 1);
...
function httpGET(url) {
  return new Promise((resolve, reject) => {
    import_node_https.default.get(url, {
      timeout: 5e3
    }, (res) => {
      let data = "";
      res.on("data", (chunk) => {
...
var DDoSProtection = class DDoSProtection2 {
  ...
  async getGeoLocation(ip, req) {
    ...
    const lookedUpIP = import_geoip_lite.default.lookup(ip);
...
var createDbConnection = /* @__PURE__ */ __name(async () => {
  try {
...
var db;
var initDatabase = /* @__PURE__ */ __name(async () => {
...
var authenticateToken = /* @__PURE__ */ __name(async (req, res, next) => {
  ...
  try {
    ...
    const [rows] = await db.execute("SELECT id, username, email, is_active FROM users WHERE id = ? AND is_active = TRUE", [
      decoded.userId
    ]);
...
var generateTokens = /* @__PURE__ */ __name((user) => {
  const payload = {
    userId: user.id,
    username: user.username,
    email: user.email
  };
  const accessToken = import_jsonwebtoken.default.sign(payload, JWT_SECRET, {
    expiresIn: "15m"
  });
  const refreshToken = import_jsonwebtoken.default.sign(payload, JWT_REFRESH_SECRET, {
    expiresIn: "7d"
  });
  return {
    accessToken,
...
var hashPassword = /* @__PURE__ */ __name(async (password) => {
  return await import_bcrypt.default.hash(password, 12);
}, "hashPassword");
var comparePassword = /* @__PURE__ */ __name(async (password, hash) => {
  return await import_bcrypt.default.compare(password, hash);
}, "comparePassword");
var validateSignup = [
  (0, import_express_validator.body)("username").isLength({
    min: 3,
    max: 50
  }).withMessage("Username must be 3-50 characters").matches(/^[a-zA-Z0-9_]+$/).withMessage("Username can only contain letters, numbers, and underscores"),
  (0, import_express_validator.body)("email").isEmail().withMessage("Please provide a valid email").normalizeEmail(),
  (0, import_express_validator.body)("password").isLength({
    min: 6
  })
];
var validateLogin = [
...
app.post("/auth/signup", validateSignup, async (req, res) => {
  try {
    console.log("\u{1F4DD} Signup request received:", {
      body: req.body,
...
app.post("/auth/login", validateLogin, async (req, res) => {
  try {
    ...
    const [users] = await db.execute("SELECT id, username, email, password, is_active FROM users WHERE email = ?", [
      email
    ]);
...
app.post("/auth/logout", authenticateToken, async (_req, res) => {
  try {
...
app.get("/auth/me", authenticateToken, async (req, res) => {
  try {
    const [users] = await db.execute("SELECT id, username, email, created_at, updated_at FROM users WHERE id = ?", [
      req.user.userId
    ]);
    if (!Array.isArray(users) || users.length === 0) {
...
app.post("/auth/refresh", async (req, res) => {
  try {
    ...
    const [users] = await db.execute("SELECT id, username, email, is_active FROM users WHERE id = ? AND is_active = TRUE", [
      decoded.userId
    ]);
    if (!Array.isArray(users) || users.length === 0) {
      return res.status(401).json({
...
app.put("/user/info", authenticateToken, [
  (0, import_express_validator.body)("username").optional().isLength({
    min: 3,
    max: 50
  }).matches(/^[a-zA-Z0-9_]+$/),
...
app.delete("/user/account", authenticateToken, async (req, res) => {
  try {
    const userId = req.user.userId;
    await db.execute("UPDATE users SET is_active = FALSE, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [
      userId
    ]);
    res.clearCookie("refreshToken");
    res.json({
...
app.get("/logs", authenticateToken, [
  (0, import_express_validator.query)("page").optional().isInt({
    min: 1
  }),
  (0, import_express_validator.query)("limit").optional().isInt({
    min: 1,
    max: 100
  }),
  (0, import_express_validator.query)("attack_type").optional().isString(),
  (0, import_express_validator.query)("start_date").optional().isISO8601(),
  (0, import_express_validator.query)("end_date").optional().isISO8601()
], async (req, res) => {
  try {
    const errors = (0, import_express_validator.validationResult)(req);
    if (!errors.isEmpty()) {
...
app.get("/logs/recent", authenticateToken, [
  (0, import_express_validator.query)("limit").optional().isInt({
    min: 1,
    max: 100
  })
  ...
  try {
    ...
    const [logs] = await db.execute("SELECT * FROM attack_logs ORDER BY timestamp DESC LIMIT ?", [
      limit
    ]);
    res.json({
      logs
...
app.get("/logs/stats", authenticateToken, async (_req, res) => {
  try {
...
app.get("/logs/type/:attackType", authenticateToken, [
  (0, import_express_validator.query)("page").optional().isInt({
    min: 1
  }),
  (0, import_express_validator.query)("limit").optional().isInt({
  ...
  try {
    ...
    const limit = parseInt(req.query["limit"]) || 50;
    const offset = (page - 1) * limit;
    const [countResult] = await db.execute("SELECT COUNT(*) as total FROM attack_logs WHERE attack_type = ?", [
      attackType
    ]);
    const total = countResult[0].total;
...
app.delete("/logs/cleanup", authenticateToken, [
  (0, import_express_validator.body)("days").isInt({
    min: 1,
    max: 365
  })
], async (req, res) => {
  try {
    ...
    const { days } = req.body;
    const [result] = await db.execute("DELETE FROM attack_logs WHERE timestamp < DATE_SUB(NOW(), INTERVAL ? DAY)", [
      days
    ]);
